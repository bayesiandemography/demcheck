...
#' @export
#' @rdname composite
chk_break_min_max_date <- function(break_min, break_max, unit) {
    val <- chk_is_length_1(x = break_min,
                           name = "break_min")
    if (!isTRUE(val))
        return(val)
    val <- chk_is_length_1(x = break_max,
                           name = "break_max")
    if (!isTRUE(val))
        return(val)
    val <- chk_is_first_day_unit_scalar(x = break_min,
                                        name = "break_min",
                                        unit = unit)
    if (!isTRUE(val))
        return(val)
    val <- chk_is_first_day_unit_scalar(x = break_max,
                                        name = "break_max",
                                        unit = unit)
    if (!isTRUE(val))
        return(val)
