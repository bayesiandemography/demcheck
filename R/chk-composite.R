...
#' @param n An integer.
#' @param first_month An abbreviated month name,
...
chk_is_first_day_unit_vector <- function(x, name, unit) {
    ...
    if (unit_is_year) {
        month <- format(x, "%m")
        first_month <- month[[1L]]
        from <- as.Date(sprintf("%d-%s-01", min(year), first_month))
        to <- as.Date(sprintf("%d-%s-01", max(year) + 1L, first_month))
    }
    else {
